...
// Tap = one small bend nudge. Hold = an immediate nudge, then (after a short
// delay so a tap never accidentally triggers continuous bending) a steady
// stream of smaller adjustments until released. Values are in SVG/automaton
// units, independent of canvas zoom — this mirrors StateMovementController's
// step model, but every adjustment here is applied to the transition's
// stored bend offset, never to a state's position.
const TAP_STEP = 14;
...
function DirectionButton({ dir, onBend, gridArea }) {
  const onBendRef = useRef(onBend);
  onBendRef.current = onBend; // always call the latest onBend, never a stale closure
  ...
  const start = e => {
    ...
    setPressed(true);
    onBendRef.current(dir.dx * TAP_STEP * magnitude, dir.dy * TAP_STEP * magnitude);
    repeatTimeoutRef.current = setTimeout(() => {
      continuousIntervalRef.current = setInterval(() => {
        onBendRef.current(dir.dx * CONTINUOUS_STEP * magnitude, dir.dy * CONTINUOUS_STEP * magnitude);
      }, CONTINUOUS_TICK_MS);
    }, REPEAT_DELAY_MS);
  };

  // Stop on unmount (e.g. the controller closes mid-hold) so a stray
  // interval never keeps bending a transition after the button is gone.
  useEffect(() => stop, []);
...
export default function TransitionBendController({ transitionLabel, onBend, onClose }) {
  ...
  return (
    <div
      role="group"
      aria-label={`Bend transition ${transitionLabel ?? ''}`}
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div>
          <p className="text-xs font-semibold text-ink dark:text-ink-dark">
            Bending {transitionLabel}
          </p>
          <p className="text-[11px] text-ink-muted dark:text-ink-darkMuted">
            Curve: {mode === 'cardinal' ? 'Cardinal' : 'Diagonal'}
          </p>
        </div>
        <button
          type="button"
          onClick={onClose}
          aria-label="Close bend controller"
      ...
      >
        {directions.map(dir => (
          <DirectionButton key={dir.id} dir={dir} gridArea={dir.area} onBend={onBend} />
        ))}
        ...
        <button
          ...
          aria-label={`Switch to ${mode === 'cardinal' ? 'diagonal' : 'cardinal'} bend mode`}
